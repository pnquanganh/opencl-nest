...

{
  ...
  const double& dI_syn_ex = y[ DI_EXC*num_nodes + tid ];
  const double& I_syn_ex = y[ I_EXC*num_nodes + tid ];
  const double& dI_syn_in = y[ DI_INH*num_nodes + tid ];
  const double& I_syn_in = y[ I_INH*num_nodes + tid ];
  ...
  f[ V_M*num_nodes + tid ] = is_refractory
    ? 0.
    : ( -P__g_L[tid] * ( V - P__E_L[tid] ) + I_spike + I_syn_ex - I_syn_in - w
        + P__I_e[tid] + B__I_stim_[tid] ) / P__C_m[tid];

  f[ DI_EXC*num_nodes + tid ] = -dI_syn_ex / P__tau_syn_ex[tid];
  // Exc. synaptic current (pA)
  f[ I_EXC*num_nodes + tid ] = dI_syn_ex - I_syn_ex / P__tau_syn_ex[tid];

  f[ DI_INH*num_nodes + tid ] = -dI_syn_in / P__tau_syn_in[tid];
  // Inh. synaptic current (pA)
  f[ I_INH*num_nodes + tid ] = dI_syn_in - I_syn_in / P__tau_syn_in[tid];
...
__global double *V__i0_ex_,
__global double *P__C_m,
__global double *V__i0_in_,
...
P__tau_w,
    ...
    // apply spikes
    S__y_[ DI_EXC*num_nodes + tid ] += ring_buffer_get_value(spike_exc_, ring_buffer_size, num_nodes, tid, lag) * V__i0_ex_[tid];
    S__y_[ DI_INH*num_nodes + tid ] += ring_buffer_get_value(spike_inh_, ring_buffer_size, num_nodes, tid, lag) * V__i0_in_[tid];
