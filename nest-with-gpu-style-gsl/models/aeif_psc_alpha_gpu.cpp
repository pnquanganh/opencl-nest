...
nest::aeif_psc_alpha_gpu::clContext_ nest::aeif_psc_alpha_gpu::gpu_context;
bool nest::aeif_psc_alpha_gpu::is_gpu_initialized = false;

nest::aeif_psc_alpha_gpu::aeif_psc_alpha_gpu()
  : graph_size( 0 )
...
if (h_V__i0_ex_) delete[] h_V__i0_ex_;
if (h_P__C_m) delete[] h_P__C_m;
if (h_V__i0_in_) delete[] h_V__i0_in_;
...
void
nest::aeif_psc_alpha_gpu::initialize_gpu()
{
...
void
nest::aeif_psc_alpha_gpu::mass_update(std::vector< Node* > nodes, Time const& origin,
					const long from,
					const long to )
{
...
bool
nest::aeif_psc_alpha_gpu::mass_wfr_update(std::vector< Node* > nodes, Time const& origin,
					    const long from,
					    const long to )
{
...
bool
nest::aeif_psc_alpha_gpu::mass_update_( std::vector<Node *> &nodes,
					  Time const& origin,
					  const long from,
					  const long to,
					  const bool called_from_wfr_update ) // TODO: don't know yet whether we need to cover both cases here
{
  ...
  initialize_device(total_num_nodes, num_local_nodes, aeif_psc_alpha::State_::STATE_VEC_SIZE);
  set_kernel_args(this->gpu_kernel, num_local_nodes, aeif_psc_alpha::State_::STATE_VEC_SIZE);
  ...
  // 	{
  // 	  nest::aeif_psc_alpha* node = (nest::aeif_psc_alpha*)*nodeIt;
  ...
  // 	}
      ...
      copy_data_to_device(nodes, aeif_psc_alpha::State_::STATE_VEC_SIZE, first_loop, lag);
      synchronize();
      ...
      copy_data_from_device(nodes, aeif_psc_alpha::State_::STATE_VEC_SIZE, false);
  ...
  copy_data_from_device(nodes, aeif_psc_alpha::State_::STATE_VEC_SIZE, true);
  return true;
}

int
nest::aeif_psc_alpha_gpu::initialize_opencl_context()
{
  int thrd_id = kernel().vp_manager.get_thread_id();
  printf("Thread %d\n", thrd_id);
...
int
nest::aeif_psc_alpha_gpu::initialize_command_queue(clContext_ *clCxt)
{
  int thrd_id = kernel().vp_manager.get_thread_id();
...
void
nest::aeif_psc_alpha_gpu::initialize_device(int total_num_nodes, int num_local_nodes, int dimension)
{
  if (!is_initialized)
    {
...
h_spike_inh_ = new double[ring_buffer_size];
	...
	return;
      ...
      getKernel("aeif_psc_alpha", "gsl", "deliver_events", &gpu_context);
      ...
      h_V__i0_ex_ = new double[len1];
      h_P__C_m = new double[len1];
      h_V__i0_in_ = new double[len1];
      ...
      create(&gpu_context, &V__i0_ex_, len1*sizeof(double));
      create(&gpu_context, &P__C_m, len1*sizeof(double));
      create(&gpu_context, &V__i0_in_, len1*sizeof(double));
...
// void
// nest::aeif_psc_alpha_gpu::prepare_copy_to_device(std::vector< Node* > &nodes, bool called_from_wfr_update, long lag_)
// {
...
void
nest::aeif_psc_alpha_gpu::copy_data_to_device(std::vector< Node* > &nodes, int dimension, bool first_loop, long lag_)
{
  int num_nodes = nodes.size();
...
UPLOAD_1D_DATA(h_V__i0_ex_, V_.i0_ex_);
UPLOAD_1D_DATA(h_P__C_m, P_.C_m);
UPLOAD_1D_DATA(h_V__i0_in_, V_.i0_in_);
...
FINISH_1D_UPLOAD(V__i0_ex_, h_V__i0_ex_, double);
FINISH_1D_UPLOAD(P__C_m, h_P__C_m, double);
FINISH_1D_UPLOAD(V__i0_in_, h_V__i0_in_, double);
...
void
nest::aeif_psc_alpha_gpu::copy_data_from_device(std::vector< Node* > &nodes, int dimension, bool last_copy)
{
  int num_nodes = nodes.size();
...
void
nest::aeif_psc_alpha_gpu::create(clContext_ *clCxt, cl::Buffer *mem, int len)
{
  cl_int ret;
  //printf("len %d\n", len);
  if (len == 0)
...
void
nest::aeif_psc_alpha_gpu::upload(clContext_ *clCxt, void *data, cl::Buffer &gdata, int datalen)
{
...
void
nest::aeif_psc_alpha_gpu::download(clContext_ *clCxt, cl::Buffer &gdata,void *data,int data_len, int offset)
{
  cl_int ret;
...
void
nest::aeif_psc_alpha_gpu::synchronize()
{
  this->command_queue.finish();
}

int
nest::aeif_psc_alpha_gpu::getKernel(string source, string kernelName, string deliver_kernel_name, clContext_ *clCxt)
{
...
int
nest::aeif_psc_alpha_gpu::set_lag_args(cl::Kernel *kernel, long lag)
{
  cl_int ret;
...
int
nest::aeif_psc_alpha_gpu::set_kernel_args(cl::Kernel *kernel, int num_nodes, int dimension)
{
  cl_int ret;
  int arg_idx = 0;
  ...
  set_kernel(V__i0_ex_);
  set_kernel(P__C_m);
  set_kernel(V__i0_in_);
...
int
nest::aeif_psc_alpha_gpu::set_deliver_kernel_args(cl::Kernel *kernel, int num_nodes, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
  cl_int ret;
  int arg_idx = 0;
...
void
nest::aeif_psc_alpha_gpu::execute_kernel(cl::Kernel *kernel, clContext_ *clCxt, size_t num_nodes)
{
  cl_int ret;
...
void
nest::aeif_psc_alpha_gpu::fill_spike_event_buffer( Event& e)
{
}

void
nest::aeif_psc_alpha_gpu::fill_event_buffer( SecondaryEvent& e)
{
...
void
nest::aeif_psc_alpha_gpu::fill_buffer_zero_double(clContext_ *clCxt, cl::Buffer &buffer, int size)
{
  cl_double zero = 0.0;
  cl_int ret;
...
void
nest::aeif_psc_alpha_gpu::fill_buffer_zero_uint(clContext_ *clCxt, cl::Buffer &buffer, int size)
{
  cl_uint zero = 0.0;
  cl_int ret;
...
void
nest::aeif_psc_alpha_gpu::initialize_graph()
{
...
void
nest::aeif_psc_alpha_gpu::upload_event_data(double *h_event_buffer, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
...
void
nest::aeif_psc_alpha_gpu::download_event_data(int num_nodes, int buffer_size)
{
...
void
nest::aeif_psc_alpha_gpu::clear_buffer()
{
...
void
nest::aeif_psc_alpha_gpu::deliver_events(double *h_event_buffer, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
  size_t num_nodes = kernel().node_manager.size();
...
void
nest::aeif_psc_alpha_gpu::deliver_events()
{
...
void
nest::aeif_psc_alpha_gpu::copy_event_data(std::vector<Node *> nodes)
{
  // int nodeid = 0;
...
void
nest::aeif_psc_alpha_gpu::handle(index sgid, index tgid)
{
  
  connections[tgid].push_back(sgid);
}

void nest::aeif_psc_alpha_gpu::handle( SpikeEvent& e )
{
...
void nest::aeif_psc_alpha_gpu::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void nest::aeif_psc_alpha_gpu::ring_buffer_add_value(double *h_ring_buffer, long pos, double val)
{
