...

{
  ...
  // dV_m/dt
  f[ 0 ] = ( -I_leak - I_syn_exc - I_syn_inh + B__I_stim_[tid] + P__I_e[tid] )
    / P__C_m[tid];

  // d dg_exc/dt, dg_exc/dt
  f[ 1 ] = -y[ DG_EXC*num_nodes + tid ] / P__tau_synE[tid];
  f[ 2 ] = y[ DG_EXC*num_nodes + tid ] - ( y[ G_EXC*num_nodes + tid ] / P__tau_synE[tid] );

  // d dg_exc/dt, dg_exc/dt
  f[ 3 ] = -y[ DG_INH*num_nodes + tid ] / P__tau_synI[tid];
  f[ 4 ] = y[ DG_INH*num_nodes + tid ] - ( y[ G_INH*num_nodes + tid ] / P__tau_synI[tid] );
...
__global int *V__RefractoryCounts,
...
P__I_e,
    ...
    { // neuron is absolute refractory
      --S__r[tid];
      S__y[ V_M*num_nodes + tid ] = P__V_reset[tid]; // clamp potential
    }
    else
      // neuron is not absolute refractory
      if ( S__y[ V_M*num_nodes + tid ] >= P__V_th[tid] )
    {
      S__r[tid] = V__RefractoryCounts[tid];
      S__y[ V_M*num_nodes + tid ] = P__V_reset[tid];
