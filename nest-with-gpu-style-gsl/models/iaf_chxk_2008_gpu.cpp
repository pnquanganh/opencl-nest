...
nest::iaf_chxk_2008_gpu::clContext_ nest::iaf_chxk_2008_gpu::gpu_context;
bool nest::iaf_chxk_2008_gpu::is_gpu_initialized = false;

nest::iaf_chxk_2008_gpu::iaf_chxk_2008_gpu()
  : graph_size( 0 )
 ...
 , h_V__PSConInit_AHP( NULL )
 , h_P__ahp_bug( NULL )
 , h_V__PSConInit_E( NULL )
 , h_P__V_th( NULL )
 , h_V__PSConInit_I( NULL )
 , h_P__E_L( NULL )
 , h_P__I_e( NULL )
 , h_P__g_L( NULL )
  , h_B_step_( NULL )
...
if (h_P__tau_ahp) delete[] h_P__tau_ahp;
...
if (h_S__y) delete[] h_S__y;
if (h_B__step_) delete[] h_B__step_;
if (h_P__tau_synI) delete[] h_P__tau_synI;
if (h_B__I_stim_) delete[] h_B__I_stim_;
if (h_P__E_ahp) delete[] h_P__E_ahp;
if (h_B__IntegrationStep_) delete[] h_B__IntegrationStep_;
if (h_V__PSConInit_AHP) delete[] h_V__PSConInit_AHP;
if (h_P__ahp_bug) delete[] h_P__ahp_bug;
if (h_V__PSConInit_E) delete[] h_V__PSConInit_E;
if (h_P__V_th) delete[] h_P__V_th;
if (h_V__PSConInit_I) delete[] h_V__PSConInit_I;
...
void
nest::iaf_chxk_2008_gpu::initialize_gpu()
{
...
void
nest::iaf_chxk_2008_gpu::mass_update(std::vector< Node* > nodes, Time const& origin,
					const long from,
					const long to )
{
...
bool
nest::iaf_chxk_2008_gpu::mass_wfr_update(std::vector< Node* > nodes, Time const& origin,
					    const long from,
					    const long to )
{
...
bool
nest::iaf_chxk_2008_gpu::mass_update_( std::vector<Node *> &nodes,
					  Time const& origin,
					  const long from,
					  const long to,
					  const bool called_from_wfr_update ) // TODO: don't know yet whether we need to cover both cases here
{
  ...
  initialize_device(total_num_nodes, num_local_nodes, iaf_chxk_2008::State_::STATE_VEC_SIZE);
  set_kernel_args(this->gpu_kernel, num_local_nodes, iaf_chxk_2008::State_::STATE_VEC_SIZE);
  ...
  // 	{
  // 	  nest::iaf_chxk_2008* node = (nest::iaf_chxk_2008*)*nodeIt;
  ...
  // 	}
      ...
      copy_data_to_device(nodes, iaf_chxk_2008::State_::STATE_VEC_SIZE, first_loop, lag);
      synchronize();
      ...
      copy_data_from_device(nodes, iaf_chxk_2008::State_::STATE_VEC_SIZE, false);
  ...
  copy_data_from_device(nodes, iaf_chxk_2008::State_::STATE_VEC_SIZE, true);
  return true;
}

int
nest::iaf_chxk_2008_gpu::initialize_opencl_context()
{
  int thrd_id = kernel().vp_manager.get_thread_id();
  printf("Thread %d\n", thrd_id);
...
int
nest::iaf_chxk_2008_gpu::initialize_command_queue(clContext_ *clCxt)
{
  int thrd_id = kernel().vp_manager.get_thread_id();
...
void
nest::iaf_chxk_2008_gpu::initialize_device(int total_num_nodes, int num_local_nodes, int dimension)
{
  if (!is_initialized)
    {
...
h_spike_inh_ = new double[ring_buffer_size];
	...
	return;
      ...
      getKernel("iaf_chxk_2008", "gsl", "deliver_events", &gpu_context);
      ...
      create(&gpu_context, &P__tau_ahp, len1*sizeof(double));
      ...
      create(&gpu_context, &S__y, len1*7*sizeof(double));
      ...
      create(&gpu_context, &P__E_ahp, len1*sizeof(double));
      create(&gpu_context, &B__IntegrationStep_, len1*sizeof(double));
      create(&gpu_context, &V__PSConInit_AHP, len1*sizeof(double));
      create(&gpu_context, &P__ahp_bug, len1*sizeof(bool));
      create(&gpu_context, &V__PSConInit_E, len1*sizeof(double));
      create(&gpu_context, &P__V_th, len1*sizeof(double));
      create(&gpu_context, &V__PSConInit_I, len1*sizeof(double));
...
// void
// nest::iaf_chxk_2008_gpu::prepare_copy_to_device(std::vector< Node* > &nodes, bool called_from_wfr_update, long lag_)
// {
...
void
nest::iaf_chxk_2008_gpu::copy_data_to_device(std::vector< Node* > &nodes, int dimension, bool first_loop, long lag_)
{
  int num_nodes = nodes.size();
...
UPLOAD_1D_DATA(h_P__tau_ahp, P_.tau_ahp);
...
UPLOAD_2D_DATA(h_S__y, S_.y);
UPLOAD_1D_DATA(h_B__step_, B_.step_);
UPLOAD_1D_DATA(h_P__tau_synI, P_.tau_synI);
UPLOAD_1D_DATA(h_B__I_stim_, B_.I_stim_);
UPLOAD_1D_DATA(h_P__E_ahp, P_.E_ahp);
UPLOAD_1D_DATA(h_B__IntegrationStep_, B_.IntegrationStep_);
UPLOAD_1D_DATA(h_V__PSConInit_AHP, V_.PSConInit_AHP);
UPLOAD_1D_DATA(h_P__ahp_bug, P_.ahp_bug);
UPLOAD_1D_DATA(h_V__PSConInit_E, V_.PSConInit_E);
UPLOAD_1D_DATA(h_P__V_th, P_.V_th);
UPLOAD_1D_DATA(h_V__PSConInit_I, V_.PSConInit_I);
...
FINISH_1D_UPLOAD(P__tau_ahp, h_P__tau_ahp, double);
...
FINISH_2D_UPLOAD(S__y, h_S__y, 7, double);
...
FINISH_1D_UPLOAD(P__E_ahp, h_P__E_ahp, double);
FINISH_1D_UPLOAD(B__IntegrationStep_, h_B__IntegrationStep_, double);
FINISH_1D_UPLOAD(V__PSConInit_AHP, h_V__PSConInit_AHP, double);
FINISH_1D_UPLOAD(P__ahp_bug, h_P__ahp_bug, bool);
FINISH_1D_UPLOAD(V__PSConInit_E, h_V__PSConInit_E, double);
FINISH_1D_UPLOAD(P__V_th, h_P__V_th, double);
FINISH_1D_UPLOAD(V__PSConInit_I, h_V__PSConInit_I, double);
...
void
nest::iaf_chxk_2008_gpu::copy_data_from_device(std::vector< Node* > &nodes, int dimension, bool last_copy)
{
  int num_nodes = nodes.size();
...
void
nest::iaf_chxk_2008_gpu::create(clContext_ *clCxt, cl::Buffer *mem, int len)
{
  cl_int ret;
  //printf("len %d\n", len);
  if (len == 0)
...
void
nest::iaf_chxk_2008_gpu::upload(clContext_ *clCxt, void *data, cl::Buffer &gdata, int datalen)
{
...
void
nest::iaf_chxk_2008_gpu::download(clContext_ *clCxt, cl::Buffer &gdata,void *data,int data_len, int offset)
{
  cl_int ret;
...
void
nest::iaf_chxk_2008_gpu::synchronize()
{
  this->command_queue.finish();
}

int
nest::iaf_chxk_2008_gpu::getKernel(string source, string kernelName, string deliver_kernel_name, clContext_ *clCxt)
{
...
int
nest::iaf_chxk_2008_gpu::set_lag_args(cl::Kernel *kernel, long lag)
{
  cl_int ret;
...
int
nest::iaf_chxk_2008_gpu::set_kernel_args(cl::Kernel *kernel, int num_nodes, int dimension)
{
  cl_int ret;
  int arg_idx = 0;
  ...
  set_kernel(V__PSConInit_AHP);
  set_kernel(P__ahp_bug);
  set_kernel(V__PSConInit_E);
  set_kernel(P__V_th);
  set_kernel(V__PSConInit_I);
  set_kernel(P__E_L);
  set_kernel(P__I_e);
  set_kernel(P__g_L);
...
int
nest::iaf_chxk_2008_gpu::set_deliver_kernel_args(cl::Kernel *kernel, int num_nodes, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
  cl_int ret;
  int arg_idx = 0;
...
void
nest::iaf_chxk_2008_gpu::execute_kernel(cl::Kernel *kernel, clContext_ *clCxt, size_t num_nodes)
{
  cl_int ret;
...
void
nest::iaf_chxk_2008_gpu::fill_spike_event_buffer( Event& e)
{
}

void
nest::iaf_chxk_2008_gpu::fill_event_buffer( SecondaryEvent& e)
{
...
void
nest::iaf_chxk_2008_gpu::fill_buffer_zero_double(clContext_ *clCxt, cl::Buffer &buffer, int size)
{
  cl_double zero = 0.0;
  cl_int ret;
...
void
nest::iaf_chxk_2008_gpu::fill_buffer_zero_uint(clContext_ *clCxt, cl::Buffer &buffer, int size)
{
  cl_uint zero = 0.0;
  cl_int ret;
...
void
nest::iaf_chxk_2008_gpu::initialize_graph()
{
...
void
nest::iaf_chxk_2008_gpu::upload_event_data(double *h_event_buffer, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
...
void
nest::iaf_chxk_2008_gpu::download_event_data(int num_nodes, int buffer_size)
{
...
void
nest::iaf_chxk_2008_gpu::clear_buffer()
{
...
void
nest::iaf_chxk_2008_gpu::deliver_events(double *h_event_buffer, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
  size_t num_nodes = kernel().node_manager.size();
...
void
nest::iaf_chxk_2008_gpu::deliver_events()
{
...
void
nest::iaf_chxk_2008_gpu::copy_event_data(std::vector<Node *> nodes)
{
  // int nodeid = 0;
...
void
nest::iaf_chxk_2008_gpu::handle(index sgid, index tgid)
{
  
  connections[tgid].push_back(sgid);
}

void nest::iaf_chxk_2008_gpu::handle( SpikeEvent& e )
{
...
void nest::iaf_chxk_2008_gpu::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );

  // add weighted current; HEP 2002-10-04
  ring_buffer_add_value(h_currents_, e.get_rel_delivery_steps( kernel().simulation_manager.get_slice_origin() ), e.get_weight() * e.get_current());
};

void nest::iaf_chxk_2008_gpu::ring_buffer_add_value(double *h_ring_buffer, long pos, double val)
{
