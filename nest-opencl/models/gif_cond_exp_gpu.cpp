...
nest::gif_cond_exp_gpu::clContext_ nest::gif_cond_exp_gpu::gpu_context;
bool nest::gif_cond_exp_gpu::is_gpu_initialized = false;

nest::gif_cond_exp_gpu::gif_cond_exp_gpu()
  : graph_size( 0 )
...
if (h_P__g_L_) delete[] h_P__g_L_;
if (h_P__E_in_) delete[] h_P__E_in_;
if (h_P__tau_synE_) delete[] h_P__tau_synE_;
if (h_S__sfa_) delete[] h_S__sfa_;
if (h_P__E_L_) delete[] h_P__E_L_;
if (h_P__I_e_) delete[] h_P__I_e_;
if (h_S__stc_) delete[] h_S__stc_;
if (h_P__c_m_) delete[] h_P__c_m_;
if (h_S__I_stim_) delete[] h_S__I_stim_;
if (h_P__E_ex_) delete[] h_P__E_ex_;
if (h_P__tau_synI_) delete[] h_P__tau_synI_;
...
void
nest::gif_cond_exp_gpu::initialize_gpu()
{
...
void
nest::gif_cond_exp_gpu::mass_update(std::vector< Node* > nodes, Time const& origin,
					const long from,
					const long to )
{
...
bool
nest::gif_cond_exp_gpu::mass_wfr_update(std::vector< Node* > nodes, Time const& origin,
					    const long from,
					    const long to )
{
...
bool
nest::gif_cond_exp_gpu::mass_update_( std::vector<Node *> &nodes,
					  Time const& origin,
					  const long from,
					  const long to,
					  const bool called_from_wfr_update ) // TODO: don't know yet whether we need to cover both cases here
{
  ...
  initialize_device(total_num_nodes, num_local_nodes, gif_cond_exp::State_::STATE_VEC_SIZE);
  set_kernel_args(this->gpu_kernel, num_local_nodes, gif_cond_exp::State_::STATE_VEC_SIZE);
  ...
  // 	{
  // 	  nest::gif_cond_exp* node = (nest::gif_cond_exp*)*nodeIt;
  ...
  // 	}
      ...
      copy_data_to_device(nodes, gif_cond_exp::State_::STATE_VEC_SIZE, first_loop, lag);
      synchronize();
      ...
      copy_data_from_device(nodes, gif_cond_exp::State_::STATE_VEC_SIZE, false);
  ...
  copy_data_from_device(nodes, gif_cond_exp::State_::STATE_VEC_SIZE, true);
}

int
nest::gif_cond_exp_gpu::initialize_opencl_context()
{
  int thrd_id = kernel().vp_manager.get_thread_id();
  printf("Thread %d\n", thrd_id);
...
int
nest::gif_cond_exp_gpu::initialize_command_queue(clContext_ *clCxt)
{
  int thrd_id = kernel().vp_manager.get_thread_id();
...
void
nest::gif_cond_exp_gpu::initialize_device(int total_num_nodes, int num_local_nodes, int dimension)
{
  if (!is_initialized)
    {
	...
	return;
      ...
      getKernel("gif_cond_exp", "gsl", "deliver_events", &gpu_context);
      ...
      h_P__g_L_ = new double[len1];
      h_P__E_in_ = new double[len1];
      h_P__tau_synE_ = new double[len1];
      h_S__sfa_ = new double[len1];
      h_P__E_L_ = new double[len1];
      h_P__I_e_ = new double[len1];
      h_S__stc_ = new double[len1];
      h_P__c_m_ = new double[len1];
      h_S__I_stim_ = new double[len1];
      h_P__E_ex_ = new double[len1];
      h_P__tau_synI_ = new double[len1];
      ...
      create(&gpu_context, &P__g_L_, len1*sizeof(double));
      create(&gpu_context, &P__E_in_, len1*sizeof(double));
      create(&gpu_context, &P__tau_synE_, len1*sizeof(double));
      create(&gpu_context, &S__sfa_, len1*sizeof(double));
      create(&gpu_context, &P__E_L_, len1*sizeof(double));
      create(&gpu_context, &P__I_e_, len1*sizeof(double));
      create(&gpu_context, &S__stc_, len1*sizeof(double));
      create(&gpu_context, &P__c_m_, len1*sizeof(double));
      create(&gpu_context, &S__I_stim_, len1*sizeof(double));
      create(&gpu_context, &P__E_ex_, len1*sizeof(double));
      create(&gpu_context, &P__tau_synI_, len1*sizeof(double));
...
// void
// nest::gif_cond_exp_gpu::prepare_copy_to_device(std::vector< Node* > &nodes, bool called_from_wfr_update, long lag_)
// {
...
void
nest::gif_cond_exp_gpu::copy_data_to_device(std::vector< Node* > &nodes, int dimension, bool first_loop, long lag_)
{
  int num_nodes = nodes.size();
...
UPLOAD_1D_DATA(h_P__g_L_, P_.g_L_);
UPLOAD_1D_DATA(h_P__E_in_, P_.E_in_);
UPLOAD_1D_DATA(h_P__tau_synE_, P_.tau_synE_);
UPLOAD_1D_DATA(h_S__sfa_, S_.sfa_);
UPLOAD_1D_DATA(h_P__E_L_, P_.E_L_);
UPLOAD_1D_DATA(h_P__I_e_, P_.I_e_);
UPLOAD_1D_DATA(h_S__stc_, S_.stc_);
UPLOAD_1D_DATA(h_P__c_m_, P_.c_m_);
UPLOAD_1D_DATA(h_S__I_stim_, S_.I_stim_);
UPLOAD_1D_DATA(h_P__E_ex_, P_.E_ex_);
UPLOAD_1D_DATA(h_P__tau_synI_, P_.tau_synI_);
...
FINISH_1D_UPLOAD(P__g_L_, h_P__g_L_, double);
FINISH_1D_UPLOAD(P__E_in_, h_P__E_in_, double);
FINISH_1D_UPLOAD(P__tau_synE_, h_P__tau_synE_, double);
FINISH_1D_UPLOAD(S__sfa_, h_S__sfa_, double);
FINISH_1D_UPLOAD(P__E_L_, h_P__E_L_, double);
FINISH_1D_UPLOAD(P__I_e_, h_P__I_e_, double);
FINISH_1D_UPLOAD(S__stc_, h_S__stc_, double);
FINISH_1D_UPLOAD(P__c_m_, h_P__c_m_, double);
FINISH_1D_UPLOAD(S__I_stim_, h_S__I_stim_, double);
FINISH_1D_UPLOAD(P__E_ex_, h_P__E_ex_, double);
FINISH_1D_UPLOAD(P__tau_synI_, h_P__tau_synI_, double);
...
void
nest::gif_cond_exp_gpu::copy_data_from_device(std::vector< Node* > &nodes, int dimension, bool last_copy)
{
  int num_nodes = nodes.size();
...
void
nest::gif_cond_exp_gpu::create(clContext_ *clCxt, cl::Buffer *mem, int len)
{
  cl_int ret;
  //printf("len %d\n", len);
  if (len == 0)
...
void
nest::gif_cond_exp_gpu::upload(clContext_ *clCxt, void *data, cl::Buffer &gdata, int datalen)
{
...
void
nest::gif_cond_exp_gpu::download(clContext_ *clCxt, cl::Buffer &gdata,void *data,int data_len, int offset)
{
  cl_int ret;
...
void
nest::gif_cond_exp_gpu::synchronize()
{
  this->command_queue.finish();
}

int
nest::gif_cond_exp_gpu::getKernel(string source, string kernelName, string deliver_kernel_name, clContext_ *clCxt)
{
...
int
nest::gif_cond_exp_gpu::set_lag_args(cl::Kernel *kernel, long lag)
{
  cl_int ret;
...
int
nest::gif_cond_exp_gpu::set_kernel_args(cl::Kernel *kernel, int num_nodes, int dimension)
{
  cl_int ret;
  int arg_idx = 0;
  ...
  set_kernel(P__g_L_);
  set_kernel(P__E_in_);
  set_kernel(P__tau_synE_);
  set_kernel(S__sfa_);
  set_kernel(P__E_L_);
  set_kernel(P__I_e_);
  set_kernel(S__stc_);
  set_kernel(P__c_m_);
  set_kernel(S__I_stim_);
  set_kernel(P__E_ex_);
  set_kernel(P__tau_synI_);
...
int
nest::gif_cond_exp_gpu::set_deliver_kernel_args(cl::Kernel *kernel, int num_nodes, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
  cl_int ret;
  int arg_idx = 0;
...
void
nest::gif_cond_exp_gpu::execute_kernel(cl::Kernel *kernel, clContext_ *clCxt, size_t num_nodes)
{
  cl_int ret;
...
void
nest::gif_cond_exp_gpu::fill_spike_event_buffer( Event& e)
{
}

void
nest::gif_cond_exp_gpu::fill_event_buffer( SecondaryEvent& e)
{
...
void
nest::gif_cond_exp_gpu::fill_buffer_zero_double(clContext_ *clCxt, cl::Buffer &buffer, int size)
{
  cl_double zero = 0.0;
  cl_int ret;
...
void
nest::gif_cond_exp_gpu::fill_buffer_zero_uint(clContext_ *clCxt, cl::Buffer &buffer, int size)
{
  cl_uint zero = 0.0;
  cl_int ret;
...
void
nest::gif_cond_exp_gpu::initialize_graph()
{
...
void
nest::gif_cond_exp_gpu::upload_event_data(double *h_event_buffer, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
...
void
nest::gif_cond_exp_gpu::download_event_data(int num_nodes, int buffer_size)
{
...
void
nest::gif_cond_exp_gpu::clear_buffer()
{
...
void
nest::gif_cond_exp_gpu::deliver_events(double *h_event_buffer, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
  size_t num_nodes = kernel().node_manager.size();
...
void
nest::gif_cond_exp_gpu::deliver_events()
{
...
void
nest::gif_cond_exp_gpu::copy_event_data(std::vector<Node *> nodes)
{
  // int nodeid = 0;
...
void
nest::gif_cond_exp_gpu::handle(index sgid, index tgid)
{
  
  connections[tgid].push_back(sgid);
}

void nest::gif_cond_exp_gpu::handle( SpikeEvent& e )
{
...
void nest::gif_cond_exp_gpu::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void nest::gif_cond_exp_gpu::ring_buffer_add_value(double *h_ring_buffer, long pos, double val)
{
