...
model_directory = '../'
template_header = 'template.h'
template_source = 'template.cpp'
template_pre_post_gsl = 'template_pre_post_gsl.cpp'
template_build_graph = 'template_build_graph.cpp'
template_opencl = 'template_kernel.cl'
gsl_loop = """ gsl_odeiv_evolve_apply(num_nodes, dimension, e_y0,
						  e_yerr, e_dydt_in,
						  e_dydt_out,
						  r_con_state_eps_abs,
						  r_con_state_eps_rel,
...
deliver_events_code = "deliver_events(h_buffer, buffer_in, buffer_out);"
...
class Neuron_Variables:
    ...
    def host_alloc(self):
        return '      ' + self.host_buffer + ' = new ' + self.var_type + ('[len1];' if self.scalar else ('[len1 *' + str(self.array_size) + '];'))

    def device_alloc(self):
        return '      create(&gpu_context, &' + self.gpu_id + (', len1' if self.scalar else ', len1*' + str(self.array_size)) + '*sizeof(' + self.var_type + '));'
...
def gen_deliver_code(ring_buffer):
    ...
    code = code.replace('buffer_in', 'd_' + ring_buffer + '_buf')
    code = code.replace('buffer_out', 'd_' + ring_buffer)
    return code
...
def gen_gpu_header(model_name, parameter_variables, list_ring_buffers, parsed_content):
    ...
    decl_device_ring_buffers = decl_device_ring_buffers + '\n' + "\n".join(['cl::Buffer d_' + rb + ';' for rb in list_ring_buffers])
...
def gen_gpu_source(model_name, parameter_variables, list_ring_buffers, parsed_content):
    ...
    param_set_arg = "\n".join([var.set_kernel() for k,var in parameter_variables.iteritems() if not var.is_gsl_output])
    ...
    param_set_arg = "\n".join([var.set_kernel() for k, var in parameter_variables.iteritems() if var.is_gsl_output])
    ...
    while i != -1:
        ...
        handle_func = handle_func + [parsed_content[i + 16: e]]
        i = parsed_content.find('/*HANDLE START*/', e)
...
def gen_pre_post_gsl(model_name, source_content, parameter_variables):
    with open(template_pre_post_gsl, 'r') as file:
        gen_pre_post_source = file.read()

    gen_pre_post_source = gen_pre_post_source.replace('model_name', model_name)
    
    s = source_content.find('/* PRE GSL START */')
    if s != -1:
        e = source_content.find('/* PRE GSL END */')
        pre_gsl_content = source_content[s + 20: e - 1]
        pre_gsl_content = remove_comment(pre_gsl_content)
        variables = re.findall('([a-zA-Z_]*)\.([a-zA-Z_]*)', pre_gsl_content)
        for var in variables:
            var_id = var[0] + '.' + var[1]
            if var_id in parameter_variables:
                parameter_variables[var_id].is_output = True
    else:
        pre_gsl_content = ''

    s = source_content.find('/* POST GSL START */')
    if s != -1:
        e = source_content.find('/* POST GSL END */')
        post_gsl_content = source_content[s + 21: e - 1]
        post_gsl_content = remove_comment(post_gsl_content)
        variables = re.findall('([a-zA-Z_]*)\.([a-zA-Z_]*)', post_gsl_content)
        for var in variables:
            var_id = var[0] + '.' + var[1]
            if var_id in parameter_variables:
                parameter_variables[var_id].is_output = True
    ...
    gen_pre_post_source = gen_pre_post_source.replace('/* PRE GSL */', pre_gsl_content)
    gen_pre_post_source = gen_pre_post_source.replace('/* POST GSL */', post_gsl_content)

    filename = '../' + model_name + '_pre_post_gsl.cpp'
    with open(filename, 'w') as file:
        file.write(gen_pre_post_source)
...
def gen_opencl(model_name, states, parameter_variables, parsed_content, list_ring_buffers):
    ...
    parameter_input_decl = "\n".join([s.param_input_decl() for k, s in parameter_variables.iteritems() if not s.is_gsl_output])
    gen_opencl_content = gen_opencl_content.replace('/* MODEL PARAMETER INPUT DECL */', parameter_input_decl)

    parameter_gsl_input_decl = "\n".join([s.gsl_input_decl() for k, s in parameter_variables.iteritems() if not s.is_gsl_output])
    gen_opencl_content = gen_opencl_content.replace('/* MODEL PARAMETER GSL INPUT DECL */', parameter_gsl_input_decl)

    parameter_input_pass = "\n".join([s.param_input_pass() for k, s in parameter_variables.iteritems() if not s.is_gsl_output])
    ...
    if s != -1:
        ...
        gsl_body = parsed_content[s + 13: e]
        gsl_body = gsl_body.replace('/*GSL CALL*/', gsl_loop)
        gsl_body = gsl_body.replace('/*SPIKE SEND*/', 'spike_count[tid]++;\n')
        gen_opencl_content = gen_opencl_content.replace('/* GSL BODY */', gsl_body)
