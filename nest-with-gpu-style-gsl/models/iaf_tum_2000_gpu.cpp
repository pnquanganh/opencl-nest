...
nest::iaf_tum_2000_gpu::clContext_ nest::iaf_tum_2000_gpu::gpu_context;
bool nest::iaf_tum_2000_gpu::is_gpu_initialized = false;

nest::iaf_tum_2000_gpu::iaf_tum_2000_gpu()
  : graph_size( 0 )
 ...
 , h_S__i_0_( NULL )
 , h_V__RefractoryCountsAbs_( NULL )
 , h_P__Theta_( NULL )
 , h_S__V_m_( NULL )
 , h_V__P21ex_( NULL )
 , h_P__V_reset_( NULL )
...
if (h_V__RefractoryCountsAbs_) delete[] h_V__RefractoryCountsAbs_;
...
if (h_S__r_tot_) delete[] h_S__r_tot_;
...
if (h_S__r_abs_) delete[] h_S__r_abs_;
...
void
nest::iaf_tum_2000_gpu::initialize_gpu()
{
...
void
nest::iaf_tum_2000_gpu::mass_update(std::vector< Node* > nodes, Time const& origin,
					const long from,
					const long to )
{
...
bool
nest::iaf_tum_2000_gpu::mass_wfr_update(std::vector< Node* > nodes, Time const& origin,
					    const long from,
					    const long to )
{
...
bool
nest::iaf_tum_2000_gpu::mass_update_( std::vector<Node *> &nodes,
					  Time const& origin,
					  const long from,
					  const long to,
					  const bool called_from_wfr_update ) // TODO: don't know yet whether we need to cover both cases here
{
  ...
  // 	{
  // 	  nest::iaf_tum_2000* node = (nest::iaf_tum_2000*)*nodeIt;
...
int
nest::iaf_tum_2000_gpu::initialize_opencl_context()
{
  int thrd_id = kernel().vp_manager.get_thread_id();
  printf("Thread %d\n", thrd_id);
...
int
nest::iaf_tum_2000_gpu::initialize_command_queue(clContext_ *clCxt)
{
  int thrd_id = kernel().vp_manager.get_thread_id();
...
void
nest::iaf_tum_2000_gpu::initialize_device(int total_num_nodes, int num_local_nodes)
{
  if (!is_initialized)
    {
...
h_spikes_in_ = new double[ring_buffer_size];
	...
	return;
      ...
      getKernel("iaf_tum_2000", "update", "deliver_events", &gpu_context);

      int len = num_local_nodes;
      
      h_S__i_0_ = new double[len];
      h_V__RefractoryCountsAbs_ = new int[len];
      ...
      create(&gpu_context, &V__RefractoryCountsAbs_, len*sizeof(int));
      ...
      create(&gpu_context, &S__r_tot_, len*sizeof(int));
      ...
      create(&gpu_context, &S__r_abs_, len*sizeof(int));
...
// void
// nest::iaf_tum_2000_gpu::prepare_copy_to_device(std::vector< Node* > &nodes, bool called_from_wfr_update, long lag_)
// {
...
void
nest::iaf_tum_2000_gpu::copy_data_to_device(std::vector< Node* > &nodes, bool first_loop, long lag_)
{
  int num_nodes = nodes.size();
...
UPLOAD_1D_DATA(h_V__RefractoryCountsAbs_, V_.RefractoryCountsAbs_);
...
UPLOAD_1D_DATA(h_S__r_tot_, S_.r_tot_);
...
UPLOAD_1D_DATA(h_S__r_abs_, S_.r_abs_);
...
FINISH_1D_UPLOAD(V__RefractoryCountsAbs_, h_V__RefractoryCountsAbs_, int);
...
FINISH_1D_UPLOAD(S__r_tot_, h_S__r_tot_, int);
...
FINISH_1D_UPLOAD(S__r_abs_, h_S__r_abs_, int);
...
void
nest::iaf_tum_2000_gpu::copy_data_from_device(std::vector< Node* > &nodes, bool last_copy)
{
  int num_nodes = nodes.size();
...
void
nest::iaf_tum_2000_gpu::create(clContext_ *clCxt, cl::Buffer *mem, int len)
{
  cl_int ret;
  //printf("len %d\n", len);
  if (len == 0)
...
void
nest::iaf_tum_2000_gpu::upload(clContext_ *clCxt, void *data, cl::Buffer &gdata, int datalen)
{
...
void
nest::iaf_tum_2000_gpu::download(clContext_ *clCxt, cl::Buffer &gdata,void *data,int data_len, int offset)
{
  cl_int ret;
...
void
nest::iaf_tum_2000_gpu::synchronize()
{
  this->command_queue.finish();
}

int
nest::iaf_tum_2000_gpu::getKernel(string source, string kernelName, string deliver_kernel_name, clContext_ *clCxt)
{
...
int
nest::iaf_tum_2000_gpu::set_lag_args(cl::Kernel *kernel, long lag)
{
  cl_int ret;
...
int
nest::iaf_tum_2000_gpu::set_kernel_args(cl::Kernel *kernel, int num_nodes)
{
  cl_int ret;
  int arg_idx = 0;
  ...
  set_kernel(S__i_0_);
  set_kernel(V__RefractoryCountsAbs_);
...
int
nest::iaf_tum_2000_gpu::set_deliver_kernel_args(cl::Kernel *kernel, int num_nodes, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
  cl_int ret;
  int arg_idx = 0;
...
void
nest::iaf_tum_2000_gpu::execute_kernel(cl::Kernel *kernel, clContext_ *clCxt, size_t num_nodes)
{
  cl_int ret;
...
void
nest::iaf_tum_2000_gpu::fill_spike_event_buffer( Event& e)
{
}

void
nest::iaf_tum_2000_gpu::fill_event_buffer( SecondaryEvent& e)
{
...
void
nest::iaf_tum_2000_gpu::fill_buffer_zero_double(clContext_ *clCxt, cl::Buffer &buffer, int size)
{
  cl_double zero = 0.0;
  cl_int ret;
...
void
nest::iaf_tum_2000_gpu::fill_buffer_zero_uint(clContext_ *clCxt, cl::Buffer &buffer, int size)
{
  cl_uint zero = 0.0;
  cl_int ret;
...
void
nest::iaf_tum_2000_gpu::initialize_graph()
{
...
void
nest::iaf_tum_2000_gpu::upload_event_data(double *h_event_buffer, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
...
void
nest::iaf_tum_2000_gpu::download_event_data(int num_nodes, int buffer_size)
{
...
void
nest::iaf_tum_2000_gpu::clear_buffer()
{
...
void
nest::iaf_tum_2000_gpu::deliver_events(double *h_event_buffer, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
  size_t num_nodes = kernel().node_manager.size();
...
void
nest::iaf_tum_2000_gpu::deliver_events()
{
...
void
nest::iaf_tum_2000_gpu::copy_event_data(std::vector<Node *> nodes)
{
  // int nodeid = 0;
...
void
nest::iaf_tum_2000_gpu::handle(index sgid, index tgid)
{
  
  connections[tgid].push_back(sgid);
}

void nest::iaf_tum_2000_gpu::handle( SpikeEvent& e )
{
...
void nest::iaf_tum_2000_gpu::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void nest::iaf_tum_2000_gpu::ring_buffer_add_value(double *h_ring_buffer, long pos, double val)
{
