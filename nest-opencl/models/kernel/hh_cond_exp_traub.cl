...

{
  ...
  // ionic currents
  const double I_Na = P__g_Na[tid] * y[ HH_M*num_nodes + tid ] * y[ HH_M*num_nodes + tid ] * y[ HH_M*num_nodes + tid ]
    * y[ HH_H*num_nodes + tid ] * ( y[ V_M*num_nodes + tid ] - P__E_Na[tid] );
  const double I_K = P__g_K[tid] * y[ HH_N*num_nodes + tid ] * y[ HH_N*num_nodes + tid ] * y[ HH_N*num_nodes + tid ]
    * y[ HH_N*num_nodes + tid ] * ( y[ V_M*num_nodes + tid ] - P__E_K[tid] );
  const double I_L = P__g_L[tid] * ( y[ V_M*num_nodes + tid ] - P__E_L[tid] );
  ...
  // membrane potential
  f[ V_M*num_nodes + tid ] = ( -I_Na - I_K - I_L - I_syn_exc - I_syn_inh + B__I_stim_[tid]
                  + P__I_e[tid] ) / P__C_m[tid];

  // channel dynamics
  const double V = y[ V_M*num_nodes + tid ] - P__V_T[tid];

  const double alpha_n =
    0.032 * ( 15. - V ) / ( exp( ( 15. - V ) / 5. ) - 1. );
  const double beta_n = 0.5 * exp( ( 10. - V ) / 40. );
  const double alpha_m =
    0.32 * ( 13. - V ) / ( exp( ( 13. - V ) / 4. ) - 1. );
  const double beta_m =
    0.28 * ( V - 40. ) / ( exp( ( V - 40. ) / 5. ) - 1. );
  const double alpha_h = 0.128 * exp( ( 17. - V ) / 18. );
  const double beta_h = 4. / ( 1. + exp( ( 40. - V ) / 5. ) );

  f[ HH_M*num_nodes + tid ] = alpha_m - ( alpha_m + beta_m ) * y[ HH_M*num_nodes + tid ]; // m-variable
  f[ HH_H*num_nodes + tid ] = alpha_h - ( alpha_h + beta_h ) * y[ HH_H*num_nodes + tid ]; // h-variable
  f[ HH_N*num_nodes + tid ] = alpha_n - ( alpha_n + beta_n ) * y[ HH_N*num_nodes + tid ]; // n-variable

  // synapses: exponential conductance
  f[ G_EXC*num_nodes + tid ] = -y[ G_EXC*num_nodes + tid ] / P__tau_synE[tid];
  f[ G_INH*num_nodes + tid ] = -y[ G_INH*num_nodes + tid ] / P__tau_synI[tid];
...
__global double *P__g_K,
__global double *P__E_ex,
__global double *P__V_T,
__global double *P__g_L,
__global double *V__U_old_,
__global int *S__r_,
__global int *V__refractory_counts_,
__global double *P__E_L,
__global double *P__E_K,
__global double *P__I_e,
__global double *P__g_Na,
		  ...
		  int time_index)
{
...

  {
    ...
    V__U_old_[tid] = S__y_[ V_M*num_nodes + tid ];


    // adaptive step integration
    while ( tt < B__step_[tid] )
    {
...
P__g_Na,
      ...
      /*gsl_odeiv_evolve_apply( B_.e_,
        ...
        &tt,                  // from t...
        B__step_[tid],             // ...to t=t+h
        &B__IntegrationStep_[tid], // integration window (written on!)
        S__y_ )*/
        ;              // neuron state
    ...
    S__y_[ G_EXC*num_nodes + tid ] += ring_buffer_get_value(spike_exc_, ring_buffer_size, num_nodes, tid, lag, time_index);
    S__y_[ G_INH*num_nodes + tid ] += ring_buffer_get_value(spike_inh_, ring_buffer_size, num_nodes, tid, lag, time_index);
    ...
    {
      // (threshold   &&    maximum    )
      if ( S__y_[ V_M*num_nodes + tid ] >= P__V_T[tid] + 30.
        && V__U_old_[tid] > S__y_[ V_M*num_nodes + tid ] )
      {
        S__r_[tid] = V__refractory_counts_[tid];
    ...
    B__I_stim_[tid] = ring_buffer_get_value(currents_, ring_buffer_size, num_nodes, tid, lag, time_index);
