...

{
  ...
  f[ I_EXC*num_nodes + tid ] = -I_syn_ex / P__tau_syn_ex[tid]; // Exc. synaptic current (pA)

  f[ I_INH*num_nodes + tid ] = -I_syn_in / P__tau_syn_in[tid]; // Inh. synaptic current (pA)
...
P__I_e,
    ...
    S__y_[ I_EXC*num_nodes + tid ] += ring_buffer_get_value(spike_exc_, ring_buffer_size, num_nodes, tid, lag);
    S__y_[ I_INH*num_nodes + tid ] += ring_buffer_get_value(spike_inh_, ring_buffer_size, num_nodes, tid, lag);
