...

{
  ...
  const double I_syn_exc = y[ G_EXC*num_nodes + tid ] * ( y[ V_M*num_nodes + tid ] - P__E_ex_[tid] );
  const double I_syn_inh = y[ G_INH*num_nodes + tid ] * ( y[ V_M*num_nodes + tid ] - P__E_in_[tid] );
  const double I_L = P__g_L_[tid] * ( y[ V_M*num_nodes + tid ] - P__E_L_[tid] );
  const double stc = S__stc_[tid];

  // V dot
  f[ 0 ] = ( -I_L + S__I_stim_[tid] + P__I_e_[tid] - I_syn_exc - I_syn_inh
             - stc ) / P__c_m_[tid];

  f[ 1 ] = -y[ G_EXC*num_nodes + tid ] / P__tau_synE_[tid];
  f[ 2 ] = -y[ G_INH*num_nodes + tid ] / P__tau_synI_[tid];
...
__kernel void gsl(int num_nodes, int dimension,
		  ...
		  __global double *rk_state_ytmp,
		  ////
__global double *P__g_L_,
__global double *P__E_in_,
__global double *P__tau_synE_,
__global double *S__sfa_,
__global double *P__E_L_,
__global double *P__I_e_,
__global double *S__stc_,
__global double *P__c_m_,
__global double *S__I_stim_,
__global double *P__E_ex_,
__global double *P__tau_synI_,
