...

{
  ...
  const double I_sfa = y[ G_SFA*num_nodes + tid ] * ( y[ V_M*num_nodes + tid ] - P__E_sfa[tid] );
  const double I_rr = y[ G_RR*num_nodes + tid ] * ( y[ V_M*num_nodes + tid ] - P__E_rr[tid] );

  // V dot
  f[ V_M*num_nodes + tid ] = ( -I_L + B__I_stim_[tid] + P__I_e[tid] - I_syn_exc - I_syn_inh
                  - I_sfa - I_rr ) / P__C_m[tid];

  f[ G_EXC*num_nodes + tid ] = -y[ G_EXC*num_nodes + tid ] / P__tau_synE[tid];
  f[ G_INH*num_nodes + tid ] = -y[ G_INH*num_nodes + tid ] / P__tau_synI[tid];

  f[ G_SFA*num_nodes + tid ] = -y[ G_SFA*num_nodes + tid ] / P__tau_sfa[tid];
  f[ G_RR*num_nodes + tid ] = -y[ G_RR*num_nodes + tid ] / P__tau_rr[tid];
...
__global double *P__q_rr,
__global double *P__g_L,
__global double *P__tau_sfa,
__global double *P__tau_rr,
__global double *P__E_rr,
__global double *P__E_ex,
__global double *P__q_sfa,
__global double *P__E_sfa,
...
V__RefractoryCounts_,
    ...
    S__y_[ G_EXC*num_nodes + tid ] += ring_buffer_get_value(spike_exc_, ring_buffer_size, num_nodes, tid, lag);
    S__y_[ G_INH*num_nodes + tid ] += ring_buffer_get_value(spike_inh_, ring_buffer_size, num_nodes, tid, lag);
    ...
    { // neuron is absolute refractory
      ...
      if ( S__y_[ V_M*num_nodes + tid ] >= P__V_th_[tid] )
    {
      ...
      S__y_[ G_SFA*num_nodes + tid ] += P__q_sfa[tid];
      S__y_[ G_RR*num_nodes + tid ] += P__q_rr[tid];
