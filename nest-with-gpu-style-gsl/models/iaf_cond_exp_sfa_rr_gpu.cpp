#include "iaf_cond_exp_sfa_rr_gpu.h"
...
nest::iaf_cond_exp_sfa_rr_gpu::clContext_ nest::iaf_cond_exp_sfa_rr_gpu::gpu_context;
bool nest::iaf_cond_exp_sfa_rr_gpu::is_gpu_initialized = false;

nest::iaf_cond_exp_sfa_rr_gpu::iaf_cond_exp_sfa_rr_gpu()
  : graph_size( 0 )
...
if (h_P__q_rr) delete[] h_P__q_rr;
if (h_P__g_L) delete[] h_P__g_L;
if (h_P__tau_sfa) delete[] h_P__tau_sfa;
if (h_P__tau_rr) delete[] h_P__tau_rr;
if (h_P__E_rr) delete[] h_P__E_rr;
if (h_P__E_ex) delete[] h_P__E_ex;
if (h_P__q_sfa) delete[] h_P__q_sfa;
if (h_P__E_sfa) delete[] h_P__E_sfa;
...
void
nest::iaf_cond_exp_sfa_rr_gpu::initialize_gpu()
{
...
void
nest::iaf_cond_exp_sfa_rr_gpu::mass_update(std::vector< Node* > nodes, Time const& origin,
					const long from,
					const long to )
{
...
bool
nest::iaf_cond_exp_sfa_rr_gpu::mass_wfr_update(std::vector< Node* > nodes, Time const& origin,
					    const long from,
					    const long to )
{
...
bool
nest::iaf_cond_exp_sfa_rr_gpu::mass_update_( std::vector<Node *> &nodes,
					  Time const& origin,
					  const long from,
					  const long to,
					  const bool called_from_wfr_update ) // TODO: don't know yet whether we need to cover both cases here
{
  ...
  initialize_device(total_num_nodes, num_local_nodes, iaf_cond_exp_sfa_rr::State_::STATE_VEC_SIZE);
  set_kernel_args(this->gpu_kernel, num_local_nodes, iaf_cond_exp_sfa_rr::State_::STATE_VEC_SIZE);
  ...
  // 	{
  // 	  nest::iaf_cond_exp_sfa_rr* node = (nest::iaf_cond_exp_sfa_rr*)*nodeIt;
  ...
  // 	}
      ...
      copy_data_to_device(nodes, iaf_cond_exp_sfa_rr::State_::STATE_VEC_SIZE, first_loop, lag);
      synchronize();
      ...
      copy_data_from_device(nodes, iaf_cond_exp_sfa_rr::State_::STATE_VEC_SIZE, false);
  ...
  copy_data_from_device(nodes, iaf_cond_exp_sfa_rr::State_::STATE_VEC_SIZE, true);
  return true;
}

int
nest::iaf_cond_exp_sfa_rr_gpu::initialize_opencl_context()
{
  int thrd_id = kernel().vp_manager.get_thread_id();
  printf("Thread %d\n", thrd_id);
...
int
nest::iaf_cond_exp_sfa_rr_gpu::initialize_command_queue(clContext_ *clCxt)
{
  int thrd_id = kernel().vp_manager.get_thread_id();
...
void
nest::iaf_cond_exp_sfa_rr_gpu::initialize_device(int total_num_nodes, int num_local_nodes, int dimension)
{
  if (!is_initialized)
    {
...
h_spike_inh_ = new double[ring_buffer_size];
	...
	return;
      ...
      getKernel("iaf_cond_exp_sfa_rr", "gsl", "deliver_events", &gpu_context);
      ...
      h_P__V_th_ = new double[len1];
      h_P__tau_synI = new double[len1];
      h_P__q_rr = new double[len1];
      h_P__g_L = new double[len1];
      h_P__tau_sfa = new double[len1];
      h_P__tau_rr = new double[len1];
      h_P__E_rr = new double[len1];
      h_P__E_ex = new double[len1];
      h_P__q_sfa = new double[len1];
      h_P__E_sfa = new double[len1];
      h_B__step_ = new double[len1];
      h_P__V_reset_ = new double[len1];
      h_V__RefractoryCounts_ = new int[len1];
      ...
      create(&gpu_context, &S__y_, len1*5*sizeof(double));
      ...
      create(&gpu_context, &P__V_th_, len1*sizeof(double));
      create(&gpu_context, &P__tau_synI, len1*sizeof(double));
      create(&gpu_context, &P__q_rr, len1*sizeof(double));
      create(&gpu_context, &P__g_L, len1*sizeof(double));
      create(&gpu_context, &P__tau_sfa, len1*sizeof(double));
      create(&gpu_context, &P__tau_rr, len1*sizeof(double));
      create(&gpu_context, &P__E_rr, len1*sizeof(double));
      create(&gpu_context, &P__E_ex, len1*sizeof(double));
      create(&gpu_context, &P__q_sfa, len1*sizeof(double));
      create(&gpu_context, &P__E_sfa, len1*sizeof(double));
      create(&gpu_context, &B__step_, len1*sizeof(double));
      create(&gpu_context, &P__V_reset_, len1*sizeof(double));
      create(&gpu_context, &V__RefractoryCounts_, len1*sizeof(int));
...
// void
// nest::iaf_cond_exp_sfa_rr_gpu::prepare_copy_to_device(std::vector< Node* > &nodes, bool called_from_wfr_update, long lag_)
// {
...
void
nest::iaf_cond_exp_sfa_rr_gpu::copy_data_to_device(std::vector< Node* > &nodes, int dimension, bool first_loop, long lag_)
{
  int num_nodes = nodes.size();
...
UPLOAD_1D_DATA(h_P__q_rr, P_.q_rr);
UPLOAD_1D_DATA(h_P__g_L, P_.g_L);
UPLOAD_1D_DATA(h_P__tau_sfa, P_.tau_sfa);
UPLOAD_1D_DATA(h_P__tau_rr, P_.tau_rr);
UPLOAD_1D_DATA(h_P__E_rr, P_.E_rr);
UPLOAD_1D_DATA(h_P__E_ex, P_.E_ex);
UPLOAD_1D_DATA(h_P__q_sfa, P_.q_sfa);
UPLOAD_1D_DATA(h_P__E_sfa, P_.E_sfa);
...
FINISH_2D_UPLOAD(S__y_, h_S__y_, 5, double);
...
FINISH_1D_UPLOAD(P__q_rr, h_P__q_rr, double);
FINISH_1D_UPLOAD(P__g_L, h_P__g_L, double);
FINISH_1D_UPLOAD(P__tau_sfa, h_P__tau_sfa, double);
FINISH_1D_UPLOAD(P__tau_rr, h_P__tau_rr, double);
FINISH_1D_UPLOAD(P__E_rr, h_P__E_rr, double);
FINISH_1D_UPLOAD(P__E_ex, h_P__E_ex, double);
FINISH_1D_UPLOAD(P__q_sfa, h_P__q_sfa, double);
FINISH_1D_UPLOAD(P__E_sfa, h_P__E_sfa, double);
...
void
nest::iaf_cond_exp_sfa_rr_gpu::copy_data_from_device(std::vector< Node* > &nodes, int dimension, bool last_copy)
{
  int num_nodes = nodes.size();
...
void
nest::iaf_cond_exp_sfa_rr_gpu::create(clContext_ *clCxt, cl::Buffer *mem, int len)
{
  cl_int ret;
  //printf("len %d\n", len);
  if (len == 0)
...
void
nest::iaf_cond_exp_sfa_rr_gpu::upload(clContext_ *clCxt, void *data, cl::Buffer &gdata, int datalen)
{
...
void
nest::iaf_cond_exp_sfa_rr_gpu::download(clContext_ *clCxt, cl::Buffer &gdata,void *data,int data_len, int offset)
{
  cl_int ret;
...
void
nest::iaf_cond_exp_sfa_rr_gpu::synchronize()
{
  this->command_queue.finish();
}

int
nest::iaf_cond_exp_sfa_rr_gpu::getKernel(string source, string kernelName, string deliver_kernel_name, clContext_ *clCxt)
{
...
int
nest::iaf_cond_exp_sfa_rr_gpu::set_lag_args(cl::Kernel *kernel, long lag)
{
  cl_int ret;
...
int
nest::iaf_cond_exp_sfa_rr_gpu::set_kernel_args(cl::Kernel *kernel, int num_nodes, int dimension)
{
  cl_int ret;
  int arg_idx = 0;
  ...
  set_kernel(P__q_rr);
  set_kernel(P__g_L);
  set_kernel(P__tau_sfa);
  set_kernel(P__tau_rr);
  set_kernel(P__E_rr);
  set_kernel(P__E_ex);
  set_kernel(P__q_sfa);
  set_kernel(P__E_sfa);
...
int
nest::iaf_cond_exp_sfa_rr_gpu::set_deliver_kernel_args(cl::Kernel *kernel, int num_nodes, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
  cl_int ret;
  int arg_idx = 0;
...
void
nest::iaf_cond_exp_sfa_rr_gpu::execute_kernel(cl::Kernel *kernel, clContext_ *clCxt, size_t num_nodes)
{
  cl_int ret;
...
void
nest::iaf_cond_exp_sfa_rr_gpu::fill_spike_event_buffer( Event& e)
{
}

void
nest::iaf_cond_exp_sfa_rr_gpu::fill_event_buffer( SecondaryEvent& e)
{
...
void
nest::iaf_cond_exp_sfa_rr_gpu::fill_buffer_zero_double(clContext_ *clCxt, cl::Buffer &buffer, int size)
{
  cl_double zero = 0.0;
  cl_int ret;
...
void
nest::iaf_cond_exp_sfa_rr_gpu::fill_buffer_zero_uint(clContext_ *clCxt, cl::Buffer &buffer, int size)
{
  cl_uint zero = 0.0;
  cl_int ret;
...
void
nest::iaf_cond_exp_sfa_rr_gpu::initialize_graph()
{
...
void
nest::iaf_cond_exp_sfa_rr_gpu::upload_event_data(double *h_event_buffer, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
...
void
nest::iaf_cond_exp_sfa_rr_gpu::download_event_data(int num_nodes, int buffer_size)
{
...
void
nest::iaf_cond_exp_sfa_rr_gpu::clear_buffer()
{
...
void
nest::iaf_cond_exp_sfa_rr_gpu::deliver_events(double *h_event_buffer, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
  size_t num_nodes = kernel().node_manager.size();
...
void
nest::iaf_cond_exp_sfa_rr_gpu::deliver_events()
{
...
void
nest::iaf_cond_exp_sfa_rr_gpu::copy_event_data(std::vector<Node *> nodes)
{
  // int nodeid = 0;
...
void
nest::iaf_cond_exp_sfa_rr_gpu::handle(index sgid, index tgid)
{
  
  connections[tgid].push_back(sgid);
}

void nest::iaf_cond_exp_sfa_rr_gpu::handle( SpikeEvent& e )
{
...
void nest::iaf_cond_exp_sfa_rr_gpu::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void nest::iaf_cond_exp_sfa_rr_gpu::ring_buffer_add_value(double *h_ring_buffer, long pos, double val)
{
