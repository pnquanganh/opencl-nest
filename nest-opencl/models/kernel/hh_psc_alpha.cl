...
double *P__g_K,
double *P__g_L,
double *V__PSCurrInit_I_,
int *S__r_,
double *V__PSCurrInit_E_,
double *P__E_L,
double *P__E_K,
double *P__I_e,
double *P__g_Na,
...

{
  ...
  const double& V = y[ V_M*num_nodes + tid ];
  const double& m = y[ HH_M*num_nodes + tid ];
  const double& h = y[ HH_H*num_nodes + tid ];
  const double& n = y[ HH_N*num_nodes + tid ];
  const double& dI_ex = y[ DI_EXC*num_nodes + tid ];
  const double& I_ex = y[ I_EXC*num_nodes + tid ];
  const double& dI_in = y[ DI_INH*num_nodes + tid ];
  const double& I_in = y[ I_INH*num_nodes + tid ];

  const double alpha_n =
    ( 0.01 * ( V + 55. ) ) / ( 1. - exp( -( V + 55. ) / 10. ) );
  const double beta_n = 0.125 * exp( -( V + 65. ) / 80. );
  const double alpha_m =
    ( 0.1 * ( V + 40. ) ) / ( 1. - exp( -( V + 40. ) / 10. ) );
  const double beta_m = 4. * exp( -( V + 65. ) / 18. );
  const double alpha_h = 0.07 * exp( -( V + 65. ) / 20. );
  const double beta_h = 1. / ( 1. + exp( -( V + 35. ) / 10. ) );

  const double I_Na = P__g_Na[tid] * m * m * m * h * ( V - P__E_Na[tid] );
  const double I_K = P__g_K[tid] * n * n * n * n * ( V - P__E_K[tid] );
  const double I_L = P__g_L[tid] * ( V - P__E_L[tid] );

  // V dot -- synaptic input are currents, inhib current is negative
  f[ V_M*num_nodes + tid ] = ( -( I_Na + I_K + I_L ) + B__I_stim_[tid] + P__I_e[tid] + I_ex
                  + I_in ) / P__C_m[tid];

  // channel dynamics
  f[ HH_M*num_nodes + tid ] =
    alpha_m * ( 1 - y[ HH_M*num_nodes + tid ] ) - beta_m * y[ HH_M*num_nodes + tid ]; // m-variable
  f[ HH_H*num_nodes + tid ] =
    alpha_h * ( 1 - y[ HH_H*num_nodes + tid ] ) - beta_h * y[ HH_H*num_nodes + tid ]; // h-variable
  f[ HH_N*num_nodes + tid ] =
    alpha_n * ( 1 - y[ HH_N*num_nodes + tid ] ) - beta_n * y[ HH_N*num_nodes + tid ]; // n-variable

  // synapses: alpha functions
  f[ DI_EXC*num_nodes + tid ] = -dI_ex / P__tau_synE[tid];
  f[ I_EXC*num_nodes + tid ] = dI_ex - ( I_ex / P__tau_synE[tid] );
  f[ DI_INH*num_nodes + tid ] = -dI_in / P__tau_synI[tid];
  f[ I_INH*num_nodes + tid ] = dI_in - ( I_in / P__tau_synI[tid] );
...
__global double *V__PSCurrInit_I_,
__global int *S__r_,
__global double *V__PSCurrInit_E_,
		  ...
		  __global double *y)
{
...

  {
    ...
    double t = 0.0;
    const double U_old = S__y_[ V_M*num_nodes + tid ];
...
P__g_Na,
    ...
    S__y_[ DI_EXC*num_nodes + tid ] +=
      ring_buffer_get_value(spike_exc_, ring_buffer_size, num_nodes, tid, lag) * V__PSCurrInit_E_[tid];
    S__y_[ DI_INH*num_nodes + tid ] +=
      ring_buffer_get_value(spike_inh_, ring_buffer_size, num_nodes, tid, lag) * V__PSCurrInit_I_[tid];
    ...
    if ( S__r_[tid] > 0 )
    {
      ...
      // (    threshold    &&     maximum       )
      if ( S__y_[ V_M*num_nodes + tid ] >= 0 && U_old > S__y_[ V_M*num_nodes + tid ] )
    {
