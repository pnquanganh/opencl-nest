...
bool nest::model_name_gpu::is_gpu_initialized = false;
...
void
nest::model_name_gpu::mass_update(std::vector< Node* > nodes, Time const& origin,
					const long from,
					const long to )
{
...
bool
nest::model_name_gpu::mass_wfr_update(std::vector< Node* > nodes, Time const& origin,
					    const long from,
					    const long to )
{
...
bool
nest::model_name_gpu::mass_update_( std::vector<Node *> &nodes,
					  Time const& origin,
					  const long from,
					  const long to,
					  const bool called_from_wfr_update ) // TODO: don't know yet whether we need to cover both cases here
{
  ...
  initialize_device(total_num_nodes, num_local_nodes, model_name::State_::STATE_VEC_SIZE);
  set_kernel_args(this->gpu_kernel, num_local_nodes, model_name::State_::STATE_VEC_SIZE);
  ...
  // 	}
      ...
      copy_data_to_device(nodes, model_name::State_::STATE_VEC_SIZE, first_loop, lag);
      synchronize();
      ...
      copy_data_from_device(nodes, model_name::State_::STATE_VEC_SIZE, false);
  ...
  copy_data_from_device(nodes, model_name::State_::STATE_VEC_SIZE, true);
  return true;
}
...
int
nest::model_name_gpu::initialize_command_queue(clContext_ *clCxt)
{
  int thrd_id = kernel().vp_manager.get_thread_id();
...
void
nest::model_name_gpu::initialize_device(int total_num_nodes, int num_local_nodes, int dimension)
{
  if (!is_initialized)
    {
	...
	return;
      ...
      getKernel("model_name", "gsl", "deliver_events", &gpu_context);
...
void
nest::model_name_gpu::copy_data_to_device(std::vector< Node* > &nodes, int dimension, bool first_loop, long lag_)
{
  int num_nodes = nodes.size();
...
void
nest::model_name_gpu::copy_data_from_device(std::vector< Node* > &nodes, int dimension, bool last_copy)
{
  int num_nodes = nodes.size();
...
int
nest::model_name_gpu::getKernel(string source, string kernelName, string deliver_kernel_name, clContext_ *clCxt)
{
...
int
nest::model_name_gpu::set_kernel_args(cl::Kernel *kernel, int num_nodes, int dimension)
{
  cl_int ret;
  int arg_idx = 0;
...
int
nest::model_name_gpu::set_deliver_kernel_args(cl::Kernel *kernel, int num_nodes, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
  cl_int ret;
  int arg_idx = 0;
...
void
nest::model_name_gpu::initialize_graph()
{
...
void
nest::model_name_gpu::upload_event_data(double *h_event_buffer, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
...
void
nest::model_name_gpu::download_event_data(int num_nodes, int buffer_size)
{
...
void
nest::model_name_gpu::deliver_events(double *h_event_buffer, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
  size_t num_nodes = kernel().node_manager.size();
...
void
nest::model_name_gpu::handle(index sgid, index tgid)
{
...
void nest::model_name_gpu::ring_buffer_add_value(double *h_ring_buffer, long pos, double val)
{
