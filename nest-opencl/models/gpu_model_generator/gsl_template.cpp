					  ...
					  const bool called_from_wfr_update ) // TODO: don't know yet whether we need to cover both cases here
{
  ...
  initialize_device(model_name::State_::STATE_VEC_SIZE);
  ...
  if (not is_data_ready)
    {
      copy_data_to_device(nodes, model_name::State_::STATE_VEC_SIZE);
      is_data_ready = true;
    }
...
void
nest::model_name_gpu::initialize_device(int dimension)
{
  if (!is_initialized)
    {
...
void
nest::model_name_gpu::copy_data_to_device(const std::vector< Node* > &nodes, int dimension)
{
  int num_nodes = nodes.size();
...
void
nest::model_name_gpu::copy_data_from_device(const std::vector< Node* > &nodes, int dimension, bool last_copy)
{
  int num_nodes = nodes.size();
...
nest::model_name_gpu::initialize()
{
  ...
  getKernel("model_name", "update", "deliver_events_stdp_pl", &gpu_context);
}
