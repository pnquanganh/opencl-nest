...
nest::iaf_psc_delta_gpu::clContext_ nest::iaf_psc_delta_gpu::gpu_context;
bool nest::iaf_psc_delta_gpu::is_gpu_initialized = false;

nest::iaf_psc_delta_gpu::iaf_psc_delta_gpu()
  : graph_size( 0 )
 ...
 , h_S__refr_spikes_buffer_( NULL )
 , h_V__RefractoryCounts_( NULL )
 , h_S__y3_( NULL )
 , h_V__P33_( NULL )
 , h_P__with_refr_input_( NULL )
 , h_P__V_reset_( NULL )
 , h_P__V_th_( NULL )
 , h_P__I_e_( NULL )
 , h_S__r_( NULL )
...
if (h_S__refr_spikes_buffer_) delete[] h_S__refr_spikes_buffer_;
if (h_V__RefractoryCounts_) delete[] h_V__RefractoryCounts_;
if (h_S__y3_) delete[] h_S__y3_;
if (h_V__P33_) delete[] h_V__P33_;
if (h_P__with_refr_input_) delete[] h_P__with_refr_input_;
...
if (h_P__tau_m_) delete[] h_P__tau_m_;
...
void
nest::iaf_psc_delta_gpu::initialize_gpu()
{
...
void
nest::iaf_psc_delta_gpu::mass_update(std::vector< Node* > nodes, Time const& origin,
					const long from,
					const long to )
{
...
bool
nest::iaf_psc_delta_gpu::mass_wfr_update(std::vector< Node* > nodes, Time const& origin,
					    const long from,
					    const long to )
{
...
bool
nest::iaf_psc_delta_gpu::mass_update_( std::vector<Node *> &nodes,
					  Time const& origin,
					  const long from,
					  const long to,
					  const bool called_from_wfr_update ) // TODO: don't know yet whether we need to cover both cases here
{
  ...
  // 	{
  // 	  nest::iaf_psc_delta* node = (nest::iaf_psc_delta*)*nodeIt;
...
int
nest::iaf_psc_delta_gpu::initialize_opencl_context()
{
  int thrd_id = kernel().vp_manager.get_thread_id();
  printf("Thread %d\n", thrd_id);
...
int
nest::iaf_psc_delta_gpu::initialize_command_queue(clContext_ *clCxt)
{
  int thrd_id = kernel().vp_manager.get_thread_id();
...
void
nest::iaf_psc_delta_gpu::initialize_device(int total_num_nodes, int num_local_nodes)
{
  if (!is_initialized)
    {
...
h_spikes_ = new double[ring_buffer_size];
	...
	return;
      ...
      getKernel("iaf_psc_delta", "update", "deliver_events", &gpu_context);

      int len = num_local_nodes;
      
      h_S__refr_spikes_buffer_ = new double[len];
      h_V__RefractoryCounts_ = new int[len];
      h_S__y3_ = new double[len];
      h_V__P33_ = new double[len];
      h_P__with_refr_input_ = new bool[len];
      ...
      h_P__tau_m_ = new double[len];
      h_spike_count = new unsigned int[len];

      create(&gpu_context, &S__refr_spikes_buffer_, len*sizeof(double));
      ...
      create(&gpu_context, &P__with_refr_input_, len*sizeof(bool));
      ...
      create(&gpu_context, &P__tau_m_, len*sizeof(double));
...
// void
// nest::iaf_psc_delta_gpu::prepare_copy_to_device(std::vector< Node* > &nodes, bool called_from_wfr_update, long lag_)
// {
...
void
nest::iaf_psc_delta_gpu::copy_data_to_device(std::vector< Node* > &nodes, bool first_loop, long lag_)
{
  int num_nodes = nodes.size();
...
UPLOAD_1D_DATA(h_P__tau_m_, P_.tau_m_);
  ...
  if (first_loop)
    {
FINISH_1D_UPLOAD(S__refr_spikes_buffer_, h_S__refr_spikes_buffer_, double);
...
FINISH_1D_UPLOAD(P__with_refr_input_, h_P__with_refr_input_, bool);
...
FINISH_1D_UPLOAD(P__tau_m_, h_P__tau_m_, double);
    }
...
void
nest::iaf_psc_delta_gpu::copy_data_from_device(std::vector< Node* > &nodes, bool last_copy)
{
  int num_nodes = nodes.size();
...
void
nest::iaf_psc_delta_gpu::create(clContext_ *clCxt, cl::Buffer *mem, int len)
{
  cl_int ret;
  //printf("len %d\n", len);
  if (len == 0)
...
void
nest::iaf_psc_delta_gpu::upload(clContext_ *clCxt, void *data, cl::Buffer &gdata, int datalen)
{
...
void
nest::iaf_psc_delta_gpu::download(clContext_ *clCxt, cl::Buffer &gdata,void *data,int data_len, int offset)
{
  cl_int ret;
...
void
nest::iaf_psc_delta_gpu::synchronize()
{
  this->command_queue.finish();
}

int
nest::iaf_psc_delta_gpu::getKernel(string source, string kernelName, string deliver_kernel_name, clContext_ *clCxt)
{
...
int
nest::iaf_psc_delta_gpu::set_lag_args(cl::Kernel *kernel, long lag)
{
  cl_int ret;
...
int
nest::iaf_psc_delta_gpu::set_kernel_args(cl::Kernel *kernel, int num_nodes)
{
  cl_int ret;
  int arg_idx = 0;
  ...
  set_kernel(S__refr_spikes_buffer_);
  set_kernel(V__RefractoryCounts_);
  set_kernel(S__y3_);
  set_kernel(V__P33_);
  set_kernel(P__with_refr_input_);
...
int
nest::iaf_psc_delta_gpu::set_deliver_kernel_args(cl::Kernel *kernel, int num_nodes, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
  cl_int ret;
  int arg_idx = 0;
...
void
nest::iaf_psc_delta_gpu::execute_kernel(cl::Kernel *kernel, clContext_ *clCxt, size_t num_nodes)
{
  cl_int ret;
...
void
nest::iaf_psc_delta_gpu::fill_spike_event_buffer( Event& e)
{
}

void
nest::iaf_psc_delta_gpu::fill_event_buffer( SecondaryEvent& e)
{
...
void
nest::iaf_psc_delta_gpu::fill_buffer_zero_double(clContext_ *clCxt, cl::Buffer &buffer, int size)
{
  cl_double zero = 0.0;
  cl_int ret;
...
void
nest::iaf_psc_delta_gpu::fill_buffer_zero_uint(clContext_ *clCxt, cl::Buffer &buffer, int size)
{
  cl_uint zero = 0.0;
  cl_int ret;
...
void
nest::iaf_psc_delta_gpu::initialize_graph()
{
...
void
nest::iaf_psc_delta_gpu::upload_event_data(double *h_event_buffer, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
...
void
nest::iaf_psc_delta_gpu::download_event_data(int num_nodes, int buffer_size)
{
...
void
nest::iaf_psc_delta_gpu::clear_buffer()
{
...
void
nest::iaf_psc_delta_gpu::deliver_events(double *h_event_buffer, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
  size_t num_nodes = kernel().node_manager.size();
...
void
nest::iaf_psc_delta_gpu::deliver_events()
{
...
void
nest::iaf_psc_delta_gpu::copy_event_data(std::vector<Node *> nodes)
{
  // int nodeid = 0;
...
void
nest::iaf_psc_delta_gpu::handle(index sgid, index tgid)
{
  
  connections[tgid].push_back(sgid);
}

void nest::iaf_psc_delta_gpu::handle( SpikeEvent& e )
{
...
void nest::iaf_psc_delta_gpu::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void nest::iaf_psc_delta_gpu::ring_buffer_add_value(double *h_ring_buffer, long pos, double val)
{
