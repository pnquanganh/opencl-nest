...
nest::iaf_cond_exp_gpu::clContext_ nest::iaf_cond_exp_gpu::gpu_context;
bool nest::iaf_cond_exp_gpu::is_gpu_initialized = false;

nest::iaf_cond_exp_gpu::iaf_cond_exp_gpu()
  : graph_size( 0 )
...
void
nest::iaf_cond_exp_gpu::initialize_gpu()
{
...
void
nest::iaf_cond_exp_gpu::mass_update(std::vector< Node* > nodes, Time const& origin,
					const long from,
					const long to )
{
...
bool
nest::iaf_cond_exp_gpu::mass_wfr_update(std::vector< Node* > nodes, Time const& origin,
					    const long from,
					    const long to )
{
...
bool
nest::iaf_cond_exp_gpu::mass_update_( std::vector<Node *> &nodes,
					  Time const& origin,
					  const long from,
					  const long to,
					  const bool called_from_wfr_update ) // TODO: don't know yet whether we need to cover both cases here
{
  ...
  initialize_device(total_num_nodes, num_local_nodes, iaf_cond_exp::State_::STATE_VEC_SIZE);
  set_kernel_args(this->gpu_kernel, num_local_nodes, iaf_cond_exp::State_::STATE_VEC_SIZE);
  ...
  // 	{
  // 	  nest::iaf_cond_exp* node = (nest::iaf_cond_exp*)*nodeIt;
  ...
  // 	}
      ...
      copy_data_to_device(nodes, iaf_cond_exp::State_::STATE_VEC_SIZE, first_loop, lag);
      synchronize();
      ...
      copy_data_from_device(nodes, iaf_cond_exp::State_::STATE_VEC_SIZE, false);
  ...
  copy_data_from_device(nodes, iaf_cond_exp::State_::STATE_VEC_SIZE, true);
  return true;
}

int
nest::iaf_cond_exp_gpu::initialize_opencl_context()
{
  int thrd_id = kernel().vp_manager.get_thread_id();
  printf("Thread %d\n", thrd_id);
...
int
nest::iaf_cond_exp_gpu::initialize_command_queue(clContext_ *clCxt)
{
  int thrd_id = kernel().vp_manager.get_thread_id();
...
void
nest::iaf_cond_exp_gpu::initialize_device(int total_num_nodes, int num_local_nodes, int dimension)
{
  if (!is_initialized)
    {
...
h_spike_inh_ = new double[ring_buffer_size];
	...
	return;
      ...
      getKernel("iaf_cond_exp", "gsl", "deliver_events", &gpu_context);
      ...
      create(&gpu_context, &S__y_, len1*3*sizeof(double));
...
// void
// nest::iaf_cond_exp_gpu::prepare_copy_to_device(std::vector< Node* > &nodes, bool called_from_wfr_update, long lag_)
// {
...
void
nest::iaf_cond_exp_gpu::copy_data_to_device(std::vector< Node* > &nodes, int dimension, bool first_loop, long lag_)
{
  int num_nodes = nodes.size();
...
FINISH_2D_UPLOAD(S__y_, h_S__y_, 3, double);
...
void
nest::iaf_cond_exp_gpu::copy_data_from_device(std::vector< Node* > &nodes, int dimension, bool last_copy)
{
  int num_nodes = nodes.size();
...
void
nest::iaf_cond_exp_gpu::create(clContext_ *clCxt, cl::Buffer *mem, int len)
{
  cl_int ret;
  //printf("len %d\n", len);
  if (len == 0)
...
void
nest::iaf_cond_exp_gpu::upload(clContext_ *clCxt, void *data, cl::Buffer &gdata, int datalen)
{
...
void
nest::iaf_cond_exp_gpu::download(clContext_ *clCxt, cl::Buffer &gdata,void *data,int data_len, int offset)
{
  cl_int ret;
...
void
nest::iaf_cond_exp_gpu::synchronize()
{
  this->command_queue.finish();
}

int
nest::iaf_cond_exp_gpu::getKernel(string source, string kernelName, string deliver_kernel_name, clContext_ *clCxt)
{
...
int
nest::iaf_cond_exp_gpu::set_lag_args(cl::Kernel *kernel, long lag)
{
  cl_int ret;
...
int
nest::iaf_cond_exp_gpu::set_kernel_args(cl::Kernel *kernel, int num_nodes, int dimension)
{
  cl_int ret;
  int arg_idx = 0;
...
int
nest::iaf_cond_exp_gpu::set_deliver_kernel_args(cl::Kernel *kernel, int num_nodes, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
  cl_int ret;
  int arg_idx = 0;
...
void
nest::iaf_cond_exp_gpu::execute_kernel(cl::Kernel *kernel, clContext_ *clCxt, size_t num_nodes)
{
  cl_int ret;
...
void
nest::iaf_cond_exp_gpu::fill_spike_event_buffer( Event& e)
{
}

void
nest::iaf_cond_exp_gpu::fill_event_buffer( SecondaryEvent& e)
{
...
void
nest::iaf_cond_exp_gpu::fill_buffer_zero_double(clContext_ *clCxt, cl::Buffer &buffer, int size)
{
  cl_double zero = 0.0;
  cl_int ret;
...
void
nest::iaf_cond_exp_gpu::fill_buffer_zero_uint(clContext_ *clCxt, cl::Buffer &buffer, int size)
{
  cl_uint zero = 0.0;
  cl_int ret;
...
void
nest::iaf_cond_exp_gpu::initialize_graph()
{
...
void
nest::iaf_cond_exp_gpu::upload_event_data(double *h_event_buffer, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
...
void
nest::iaf_cond_exp_gpu::download_event_data(int num_nodes, int buffer_size)
{
...
void
nest::iaf_cond_exp_gpu::clear_buffer()
{
...
void
nest::iaf_cond_exp_gpu::deliver_events(double *h_event_buffer, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
  size_t num_nodes = kernel().node_manager.size();
...
void
nest::iaf_cond_exp_gpu::deliver_events()
{
...
void
nest::iaf_cond_exp_gpu::copy_event_data(std::vector<Node *> nodes)
{
  // int nodeid = 0;
...
void
nest::iaf_cond_exp_gpu::handle(index sgid, index tgid)
{
  
  connections[tgid].push_back(sgid);
}

void nest::iaf_cond_exp_gpu::handle( SpikeEvent& e )
{
...
void nest::iaf_cond_exp_gpu::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void nest::iaf_cond_exp_gpu::ring_buffer_add_value(double *h_ring_buffer, long pos, double val)
{
