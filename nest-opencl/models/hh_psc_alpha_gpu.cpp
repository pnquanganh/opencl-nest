...
nest::hh_psc_alpha_gpu::clContext_ nest::hh_psc_alpha_gpu::gpu_context;
bool nest::hh_psc_alpha_gpu::is_gpu_initialized = false;

nest::hh_psc_alpha_gpu::hh_psc_alpha_gpu()
  : graph_size( 0 )
 ...
 , h_P__g_K( NULL )
 , h_P__g_L( NULL )
 , h_V__PSCurrInit_I_( NULL )
 , h_S__r_( NULL )
 , h_V__PSCurrInit_E_( NULL )
 , h_P__E_L( NULL )
 , h_P__E_K( NULL )
 , h_S__y_( NULL )
 , h_P__I_e( NULL )
...
if (h_V__PSCurrInit_I_) delete[] h_V__PSCurrInit_I_;
if (h_S__r_) delete[] h_S__r_;
if (h_V__PSCurrInit_E_) delete[] h_V__PSCurrInit_E_;
...
void
nest::hh_psc_alpha_gpu::initialize_gpu()
{
...
void
nest::hh_psc_alpha_gpu::mass_update(std::vector< Node* > nodes, Time const& origin,
					const long from,
					const long to )
{
...
bool
nest::hh_psc_alpha_gpu::mass_wfr_update(std::vector< Node* > nodes, Time const& origin,
					    const long from,
					    const long to )
{
...
bool
nest::hh_psc_alpha_gpu::mass_update_( std::vector<Node *> &nodes,
					  Time const& origin,
					  const long from,
					  const long to,
					  const bool called_from_wfr_update ) // TODO: don't know yet whether we need to cover both cases here
{
  ...
  initialize_device(total_num_nodes, num_local_nodes, hh_psc_alpha::State_::STATE_VEC_SIZE);
  set_kernel_args(this->gpu_kernel, num_local_nodes, hh_psc_alpha::State_::STATE_VEC_SIZE);
  ...
  // 	{
  // 	  nest::hh_psc_alpha* node = (nest::hh_psc_alpha*)*nodeIt;
  ...
  // 	}
      ...
      copy_data_to_device(nodes, hh_psc_alpha::State_::STATE_VEC_SIZE, first_loop, lag);
      synchronize();
      ...
      copy_data_from_device(nodes, hh_psc_alpha::State_::STATE_VEC_SIZE, false);
  ...
  copy_data_from_device(nodes, hh_psc_alpha::State_::STATE_VEC_SIZE, true);
  return true;
}

int
nest::hh_psc_alpha_gpu::initialize_opencl_context()
{
  int thrd_id = kernel().vp_manager.get_thread_id();
  printf("Thread %d\n", thrd_id);
...
int
nest::hh_psc_alpha_gpu::initialize_command_queue(clContext_ *clCxt)
{
  int thrd_id = kernel().vp_manager.get_thread_id();
...
void
nest::hh_psc_alpha_gpu::initialize_device(int total_num_nodes, int num_local_nodes, int dimension)
{
  if (!is_initialized)
    {
...
h_spike_inh_ = new double[ring_buffer_size];
	...
	return;
      ...
      getKernel("hh_psc_alpha", "gsl", "deliver_events", &gpu_context);
      ...
      h_V__PSCurrInit_I_ = new double[len1];
      h_S__r_ = new int[len1];
      h_V__PSCurrInit_E_ = new double[len1];
      h_P__E_L = new double[len1];
      h_P__E_K = new double[len1];
      h_S__y_ = new double[len1 *8];
      ...
      create(&gpu_context, &V__PSCurrInit_I_, len1*sizeof(double));
      create(&gpu_context, &S__r_, len1*sizeof(int));
      create(&gpu_context, &V__PSCurrInit_E_, len1*sizeof(double));
      create(&gpu_context, &P__E_L, len1*sizeof(double));
      create(&gpu_context, &P__E_K, len1*sizeof(double));
      create(&gpu_context, &S__y_, len1*8*sizeof(double));
...
// void
// nest::hh_psc_alpha_gpu::prepare_copy_to_device(std::vector< Node* > &nodes, bool called_from_wfr_update, long lag_)
// {
...
void
nest::hh_psc_alpha_gpu::copy_data_to_device(std::vector< Node* > &nodes, int dimension, bool first_loop, long lag_)
{
  int num_nodes = nodes.size();
...
UPLOAD_1D_DATA(h_V__PSCurrInit_I_, V_.PSCurrInit_I_);
UPLOAD_1D_DATA(h_S__r_, S_.r_);
UPLOAD_1D_DATA(h_V__PSCurrInit_E_, V_.PSCurrInit_E_);
...
FINISH_1D_UPLOAD(V__PSCurrInit_I_, h_V__PSCurrInit_I_, double);
FINISH_1D_UPLOAD(S__r_, h_S__r_, int);
FINISH_1D_UPLOAD(V__PSCurrInit_E_, h_V__PSCurrInit_E_, double);
FINISH_1D_UPLOAD(P__E_L, h_P__E_L, double);
FINISH_1D_UPLOAD(P__E_K, h_P__E_K, double);
FINISH_2D_UPLOAD(S__y_, h_S__y_, 8, double);
...
void
nest::hh_psc_alpha_gpu::copy_data_from_device(std::vector< Node* > &nodes, int dimension, bool last_copy)
{
  int num_nodes = nodes.size();
...
void
nest::hh_psc_alpha_gpu::create(clContext_ *clCxt, cl::Buffer *mem, int len)
{
  cl_int ret;
  //printf("len %d\n", len);
  if (len == 0)
...
void
nest::hh_psc_alpha_gpu::upload(clContext_ *clCxt, void *data, cl::Buffer &gdata, int datalen)
{
...
void
nest::hh_psc_alpha_gpu::download(clContext_ *clCxt, cl::Buffer &gdata,void *data,int data_len, int offset)
{
  cl_int ret;
...
void
nest::hh_psc_alpha_gpu::synchronize()
{
  this->command_queue.finish();
}

int
nest::hh_psc_alpha_gpu::getKernel(string source, string kernelName, string deliver_kernel_name, clContext_ *clCxt)
{
...
int
nest::hh_psc_alpha_gpu::set_lag_args(cl::Kernel *kernel, long lag)
{
  cl_int ret;
...
int
nest::hh_psc_alpha_gpu::set_kernel_args(cl::Kernel *kernel, int num_nodes, int dimension)
{
  cl_int ret;
  int arg_idx = 0;
  ...
  set_kernel(P__g_K);
  set_kernel(P__g_L);
  set_kernel(V__PSCurrInit_I_);
  set_kernel(S__r_);
  set_kernel(V__PSCurrInit_E_);
  set_kernel(P__E_L);
  set_kernel(P__E_K);
  set_kernel(P__I_e);
  set_kernel(P__g_Na);
...
int
nest::hh_psc_alpha_gpu::set_deliver_kernel_args(cl::Kernel *kernel, int num_nodes, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
  cl_int ret;
  int arg_idx = 0;
...
void
nest::hh_psc_alpha_gpu::execute_kernel(cl::Kernel *kernel, clContext_ *clCxt, size_t num_nodes)
{
  cl_int ret;
...
void
nest::hh_psc_alpha_gpu::fill_spike_event_buffer( Event& e)
{
}

void
nest::hh_psc_alpha_gpu::fill_event_buffer( SecondaryEvent& e)
{
...
void
nest::hh_psc_alpha_gpu::fill_buffer_zero_double(clContext_ *clCxt, cl::Buffer &buffer, int size)
{
  cl_double zero = 0.0;
  cl_int ret;
...
void
nest::hh_psc_alpha_gpu::fill_buffer_zero_uint(clContext_ *clCxt, cl::Buffer &buffer, int size)
{
  cl_uint zero = 0.0;
  cl_int ret;
...
void
nest::hh_psc_alpha_gpu::initialize_graph()
{
...
void
nest::hh_psc_alpha_gpu::upload_event_data(double *h_event_buffer, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
...
void
nest::hh_psc_alpha_gpu::download_event_data(int num_nodes, int buffer_size)
{
...
void
nest::hh_psc_alpha_gpu::clear_buffer()
{
...
void
nest::hh_psc_alpha_gpu::deliver_events(double *h_event_buffer, cl::Buffer &d_event_buffer_in, cl::Buffer &d_event_buffer_out)
{
  size_t num_nodes = kernel().node_manager.size();
...
void
nest::hh_psc_alpha_gpu::deliver_events()
{
...
void
nest::hh_psc_alpha_gpu::copy_event_data(std::vector<Node *> nodes)
{
  // int nodeid = 0;
...
void
nest::hh_psc_alpha_gpu::handle(index sgid, index tgid)
{
  
  connections[tgid].push_back(sgid);
}

void nest::hh_psc_alpha_gpu::handle( SpikeEvent& e )
{
...
void nest::hh_psc_alpha_gpu::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void nest::hh_psc_alpha_gpu::ring_buffer_add_value(double *h_ring_buffer, long pos, double val)
{
